...
def merge_from_config(config, config_merge, check=True):
    for k, v in config_merge.items():
        # assert k in config, f"The key {k} is not in the base config for the merge."
        if k not in config and check:
            print(f'[Error] New args {k} is added')
        config[k] = v


def merge_from_file(config, fname):
    merge_from_config(config, load(fname), False)
