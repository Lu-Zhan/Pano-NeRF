...
class PureMLP(torch.nn.Module):
    ...
    def forward(self, x, view_direction=None):
        ...
        Returns:
            ...
            raw_intrinsic: torch.Tensor(float32), with a shape of
                [batch, num_samples, 6].
        """
...
class MipNeRF(torch.nn.Module):
    ...
    def __init__(self, num_samples: int = 128,
        ...
        self.mlp = PureMLP(mlp_net_depth, mlp_net_width, mlp_net_depth_condition, mlp_net_width_condition,
        ...
        if density_activation == 'softplus':  # Density activation.
            ...
            raise NotImplementedError
        self.nor_activation = torch.nn.Tanh()  # Normal activation.
    ...
    def forward(self, 
        ...
        for i_level in range(self.num_levels):
            ...
            rgb, density = compute_graph(means_covs[0], means_covs[1], rays.viewdirs)
            ...
            if i_level == 1 and use_ort_loss:
                # surface rendering
                normalized_weights = torch.unsqueeze(weights, dim=-1) / acc.view(-1, 1, 1)

                # normal: derivative of density
                ...
                den_normal = torch.sum(normalized_weights * normals, dim=1)
                den_normal = normalize(den_normal, dim=-1)
                ...
                ret.append((comp_rgb, distance, ort_loss, den_normal))
            else:
                ret.append((comp_rgb, distance, None, torch.ones_like(comp_rgb)))
